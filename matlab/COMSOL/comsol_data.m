...
data_table = readtable('Parametrico2.csv');
...
for i=1:length(time)
    ia(i) = 3*sin(2*pi*fe*time(i));
    ib(i) = 3*sin(2*pi*fe*time(i)-2*pi/3);
    ic(i) = 3*sin(2*pi*fe*time(i)+2*pi/3);

    fa(i) = interpn(iaVec,ibVec',thVec,fluxA,ia(i),ib(i),thetar);
    fb(i) = interpn(iaVec,ibVec',thVec,fluxB,ia(i),ib(i),thetar);
    fc(i) = interpn(iaVec,ibVec',thVec,fluxC,ia(i),ib(i),thetar);

    thetae = N*thetar*pi/180;
    ...
    thetar = thetar_ant + omegar*Ts*180/pi;
    if thetar > 360/N
        thetar = thetar - 360/N;
    end
    thetar_ant = thetar;
