...
tray_contr_idq_p1p     = zeros( tamaprox, 2 );
tray_contr_thetar      = zeros( tamaprox, 1);
tray_contr_thetar_deg  = zeros( tamaprox, 1);
...
param_exp_idq_inicial = [isd_ref isq_ref];
var_sim_x = param_exp_idq_inicial';
var_sim_idq_real = param_exp_idq_inicial;
var_contr_idq_real = param_exp_idq_inicial';
...
inte_km1 = 0;
var_sim_fdq = [0;0];
var_sim_fdq(1) = interpn(idVec,iqVec',angleVec,fluxD,isd_ref,isq_ref,var_sim_thetar_deg,'linear');
var_sim_fdq(2) = interpn(idVec,iqVec',angleVec,fluxQ,isd_ref,isq_ref,var_sim_thetar_deg,'linear');
...
%                 
        % CONTROL DE CORRIENTE FCS-MPC (horizonte k+2)
        % Predicción para k+1
        var_contr_idq_p1p(1) = var_contr_idq_real(1) + (cte_contr_Tm_FSMPC/Ld)*(var_contr_vdq(1) - R_s*var_contr_idq_real(1) - omegae*Lq*var_contr_idq_real(2));
        var_contr_idq_p1p(2) = var_contr_idq_real(2) + (cte_contr_Tm_FSMPC/Lq)*(var_contr_vdq(2) - R_s*var_contr_idq_real(2) + omegae*Ld*var_contr_idq_real(1) + PM);
        % Estimación del ángulo del rotor para k+1
        ...
        for vv=1:param_NV
            % Tensión dq
            var_contr_vfase = param_vdc*param_T*XI8(vv,:)';
            var_opt_vdq = param_Mtransf*var_contr_vfase;
            % Corrientes en k+2
            var_contr_idq_p2p(1) = var_contr_idq_p1p(1) + (cte_contr_Tm_FSMPC/Ld)*(var_opt_vdq(1) - R_s*var_contr_idq_p1p(1) - omegae*Lq*var_contr_idq_p1p(2));
            var_contr_idq_p2p(2) = var_contr_idq_p1p(2) + (cte_contr_Tm_FSMPC/Lq)*(var_opt_vdq(2) - R_s*var_contr_idq_p1p(2) + omegae*Ld*var_contr_idq_p1p(1) + PM);
        ...
        tray_contr_idq_real(var_contr_contadorPM+1,:)   = var_contr_idq_real;
        ...
        tray_contr_idq_p1p(var_contr_contadorPM+1,:)    = var_contr_idq_p1p;
        tray_contr_thetar(var_contr_contadorPM+1,:)     = var_contr_thetar;
    end
    ...
    % Flujos en h+1
    var_sim_fdq = var_sim_fdq + param_sim_h*(var_contr_vdq - R_s*var_sim_x + omegae*[0 -1;1 0]*var_sim_fdq);
...
if var_contr_contadorPM < tamaprox
    ...
    tray_contr_idq_p1p( var_contr_contadorPM+1:end,:)     = [];
    tray_contr_thetar( var_contr_contadorPM+1:end,:)      = []; 
    tray_contr_thetar_deg( var_contr_contadorPM+1:end,:)  = [];
end

%% Representación de resultados
figure
hold on
plot(tray_contr_tiempoc,tray_contr_idq_refe,'--')
...
% figure
% hold on
% plot(tray_sim_tiempo,tray_sim_thetar(1:end-1,:)*N)
% hold on
% plot(tray_sim_tiempo,tray_sim_thetar_deg(1:end-1,:)*pi/180*N)
% grid on
% 
% figure
% plot(tray_sim_tiempo(1:end-1),tray_sim_torque)
% grid on
% 
% figure
...
% 
% figure
% plot(tray_sim_tiempo,tray_sim_ifase_real(1:end-1,:))
% hold on
% plot(tray_contr_tiempoc,tray_contr_ifase_real)
% grid on
% 
% figure
% plot(tray_sim_tiempo(1:end-1),var_contr_vdq_opt)
% grid on
% 
% figure
% hold on
% plot(tray_sim_tiempo,tray_sim_idq_real(1:end-1,:))
% plot(tray_contr_tiempoc,tray_contr_idq_p1p)
% % plot(tray_contr_tiempoc,tray_contr_idq_refe,'k--')
% grid on
% 
% figure
% hold on
% plot(tray_sim_tiempo,tray_sim_thetar(1:end-1,:))
% plot(tray_contr_tiempoc,tray_contr_thetar)
% grid on
% 
% figure
% hold on
% plot(tray_sim_tiempo,tray_sim_thetar_deg(1:end-1,:))
% plot(tray_contr_tiempoc,tray_contr_thetar_deg)
% grid on
