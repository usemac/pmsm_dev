...
% plot(angleVec,ifa,angleVec,ifb,angleVec,ifc)
% hold on
% plot(angleVec,ifase(1,:),'--',angleVec,ifase(2,:),'--',angleVec,ifase(3,:),'--')
% legend('i_{a}','i_{b}','i_{c}','i_{a2}','i_{b2}','i_{c2}')
% grid on
% 
% figure
% plot(angleVec,fa,angleVec,fb,angleVec,fc)
...
% legend('\phi_{a}','\phi_{b}','\phi_{c}','\phi_{a2}','\phi_{b2}','\phi_{c2}')
% grid on
...
% Flux as a function of iA, iB, iC, and thetar
% A phase
...
% Flux as function of id, iq, and thetar (creo que esta función devuelve
% los fujos haciendo la inversa de park con el signo negativo en el seno
% A phase
[dFA2dA,dFA2dB,dFA2dC,dFA2dX,iD,iQ] = ee_calculateFluxPartialDerivatives(iaVec,ibVec,icVec,angleVec*pi/180,fluxA2);
% B phase
[dFB2dA,dFB2dB,dFB2dC,dFB2dX,iD,iQ] = ee_calculateFluxPartialDerivatives(iaVec,ibVec,icVec,angleVec*pi/180,fluxB2);
% C phase
[dFC2dA,dFC2dB,dFC2dC,dFC2dX,iD,iQ] = ee_calculateFluxPartialDerivatives(iaVec,ibVec,icVec,angleVec*pi/180,fluxC2);
...
phi_ar = zeros(1,length(angleVec));
phi_br = zeros(1,length(angleVec));
phi_cr = zeros(1,length(angleVec));
...
%               1/2            1/2                     1/2];
    ...
    angulo = angleVec(i);
    % Fluxes
    fluxa(i) = interpn(iaVec,ibVec,icVec,angleVec,fluxA2,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    fluxb(i) = interpn(iaVec,ibVec,icVec,angleVec,fluxB2,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    fluxc(i) = interpn(iaVec,ibVec,icVec,angleVec,fluxC2,iabc(1,i),iabc(2,i),iabc(3,i),angulo);

    % Flux derivatives
    laa(i) = interpn(iaVec,ibVec,icVec,angleVec,dFAdA,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lab(i) = interpn(iaVec,ibVec,icVec,angleVec,dFAdB,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lac(i) = interpn(iaVec,ibVec,icVec,angleVec,dFAdC,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    phi_ar(i) = interpn(iaVec,ibVec,icVec,angleVec,dFAdX,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    
    lba(i) = interpn(iaVec,ibVec,icVec,angleVec,dFBdA,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lbb(i) = interpn(iaVec,ibVec,icVec,angleVec,dFBdB,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lbc(i) = interpn(iaVec,ibVec,icVec,angleVec,dFBdC,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    phi_br(i) = interpn(iaVec,ibVec,icVec,angleVec,dFBdX,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    
    lca(i) = interpn(iaVec,ibVec,icVec,angleVec,dFCdA,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lcb(i) = interpn(iaVec,ibVec,icVec,angleVec,dFCdB,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    lcc(i) = interpn(iaVec,ibVec,icVec,angleVec,dFCdC,iabc(1,i),iabc(2,i),iabc(3,i),angulo);
    phi_cr(i) = interpn(iaVec,ibVec,icVec,angleVec,dFCdX,iabc(1,i),iabc(2,i),iabc(3,i),angulo);

    % Flux derivatives
    laa2(i) = interp3(iQ,iD',angleVec,dFA2dA,iq,id,angulo);
    lab2(i) = interp3(iQ,iD',angleVec,dFA2dB,iq,id,angulo);
    lac2(i) = interp3(iQ,iD',angleVec,dFA2dC,iq,id,angulo);
    phi_ar2(i) = interp3(iQ,iD',angleVec,dFA2dX,iq,id,angulo);
    
    lba2(i) = interp3(iQ,iD',angleVec,dFB2dA,iq,id,angulo);
    lbb2(i) = interp3(iQ,iD',angleVec,dFB2dB,iq,id,angulo);
    lbc2(i) = interp3(iQ,iD',angleVec,dFB2dC,iq,id,angulo);
    phi_br2(i) = interp3(iQ,iD',angleVec,dFB2dX,iq,id,angulo);
    
    lca2(i) = interp3(iQ,iD',angleVec,dFC2dA,iq,id,angulo);
    lcb2(i) = interp3(iQ,iD',angleVec,dFC2dB,iq,id,angulo);
    lcc2(i) = interp3(iQ,iD',angleVec,dFC2dC,iq,id,angulo);
    phi_cr2(i) = interp3(iQ,iD',angleVec,dFC2dX,iq,id,angulo);
end
...
hold on
plot(angleVec,laa2,'--',angleVec,lbb2,'--',angleVec,lcc2,'--')
grid on
legend('l_{aa}','l_{bb}','l_{cc}','l_{aa2}','l_{bb2}','l_{cc2}')

% lab = dFA/diB; lba = dFB/diA
figure
plot(angleVec,lab,angleVec,lba)
hold on
plot(angleVec,lab2,'--',angleVec,lba2,'--')
legend('l_{ab}','l_{ba}','l_{ab2}','l_{ba2}')

% lac = dFA/diC; lca = dFC/diA
figure
plot(angleVec,lac,angleVec,lca)
hold on
plot(angleVec,lac2,'--',angleVec,lca2,'--')
legend('l_{ac}','l_{ca}','l_{ac2}','l_{ca2}')

% lbc = dFB/diC; lcb = dFC/diB
figure
plot(angleVec,lbc,angleVec,lcb)
hold on
plot(angleVec,lbc2,'--',angleVec,lcb2,'--')
legend('l_{bc}','l_{cb}','l_{bc2}','l_{cb2}')

% phi_ar = dFA/dX; phi_br = dFB/dX; phi_cr = dFC/dX
figure
plot(angleVec,phi_ar,angleVec,phi_br,angleVec,phi_cr)
hold on
plot(angleVec,phi_ar2,'--',angleVec,phi_br2,'--',angleVec,phi_cr2,'--')
grid on
legend('\phi_{ar}','\phi_{br}','\phi_{cr}','\phi_{ar2}','\phi_{br2}','\phi_{cr2}')

%% Intento de simulación del sistema conocidas vabc

var_sim_iabc = zeros(3,nd);
var_sim_iabc(:,100) = [ia_sim(100);ib_sim(100);ic_sim(100)];
...
%     phi_cr = interpn(iaVec,ibVec,icVec,angleVec,dFCdX,i_a,i_b,i_c,thetar_deg,method);
    ...
    l_aa = interpn(iQ,iD',angleVec,dFA2dA,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_ab = interpn(iQ,iD',angleVec,dFA2dB,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_ac = interpn(iQ,iD',angleVec,dFA2dC,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    phi_ar = interpn(iQ,iD',angleVec,dFA2dX,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    
    l_ba = interpn(iQ,iD',angleVec,dFB2dA,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_bb = interpn(iQ,iD',angleVec,dFB2dB,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_bc = interpn(iQ,iD',angleVec,dFB2dC,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    phi_br = interpn(iQ,iD',angleVec,dFB2dX,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    
    l_ca = interpn(iQ,iD',angleVec,dFC2dA,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_cb = interpn(iQ,iD',angleVec,dFC2dB,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    l_cc = interpn(iQ,iD',angleVec,dFC2dC,idq(2,i-1),idq(1,i-1),thetar_deg,method);
    phi_cr = interpn(iQ,iD',angleVec,dFC2dX,idq(2,i-1),idq(1,i-1),thetar_deg,method);
